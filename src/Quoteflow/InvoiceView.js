...
import { View,TextInput, Button, Text, Linking, StyleSheet,TouchableOpacity ,RefreshControl,ScrollView} from 'react-native';
...
const InvoiceView = ({route}) => {
    const{InvoiceTo,phoneno,Terms,AccountDetail,InitialAmount}=route.params;
...
<h2>Received</h2>
<div>
    <h2>${Number(InitialAmount).toLocaleString()}</h2>
</div>
</div>
...
<h2>Balance</h2>
<div>
    <h2>${Number(Amount-InitialAmount).toLocaleString()}</h2>
</div>
</div>
