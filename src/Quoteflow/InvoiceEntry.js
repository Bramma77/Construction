...
const InvoiceEntry=({route})=>{
    ...
    const reference = database().ref('Construction').child('ProjectView');
    
    const onSubmit=()=>{
       
         reference.child(`InvoiceList`).push({
        SNo:Sno,
        Description:Item,
        Length:Length,
        Breadth:Breath,
...
const deleteData = async (key) => {
    ...
    try {
      await reference.child('InvoiceList').child(key).remove();
...
useEffect(() => {
    ...
    const onValueChange = reference.child('InvoiceList').on('value', (snapshot) => {
